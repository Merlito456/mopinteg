...
def _preprocess(image_bytes: bytes) -> Image.Image:
    ...
    img = img.convert("L")
    # Invert (dark text on light background works better)
...
# ------------------------------------------------------------
PATTERNS = {
    # Node names (Globe pattern: XXX-YYY-NNN or XXX_YYY_NNN)
    "OLT_SITE": [
...
def parse_ewp(image_bytes: bytes) -> dict:
    ...
    Only returns keys where a match was found.
    """
    result = {}
    ...
    for key, patterns in PATTERNS.items():
        for pattern in patterns:
            ...
            if match:
                # Clean up spacing around slashes
                value = match.group(0)
    ...
    # Post-process: normalize some known values
...
def _post_process(result: dict) -> dict:
    ...
    # Normalize node names (use underscores)
    for key in ["AG1_NODE", "AG2_NODE", "CX600_NODE", "AGG_NODE_OM",
                "BNG_NODE_SIP_HSI", "BNG_NODE_IPOE2"]:
        if key in result:
            val = result[key].replace("-", "-")  # keep hyphens for FIO-style names
            result[key] = val
    
    # Normalize VSI names (must match exact pattern)
